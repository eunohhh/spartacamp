...
function displayGuesses(guess) {
  ...
  $remainingCount.innerHTML = String(Number($remainingCount.innerText)-1);
}
...
function makeAnswerCircle(guess) {
  ...
  showCircle(guess, CIRCLE_NAME, $answerCircleArea)
    .then(div => {
      div.id = "answerCircle"
...
function makeGuessCircle(guess) {
  ...
  // 구현3. 유저가 원의 크기로 정답을 유추하기 쉽도록 showCircle 함수를 이용해서 해당 부분을 구현해주세요,
  // showCircle 함수의 "작업"이 끝나면, 해당 div에 원의 이름을 입력해주세요
  showCircle(guess, CIRCLE_NAME, $guessCircleArea)
    .then((div) => {
      div.id = "guessCircle";
      div.append(CIRCLE_NAME);
    });
